...
var curr_data, V, src, dst; // Current data, size, source, destination
...
onload = function () {
  ...
  function createData() {
    ...
    while (src == dst) src = Math.floor(Math.random() * V);
  ...
  get_prbm.onclick = function () {
    ...
    prbm_text.innerHTML =
      "Find the least time path from <b>" +
      locations[src] +
      "</b> to <b>" +
      locations[dst] +
      "</b>";
  ...
  function dijkstra(graph, sz, src) {
    ...
    let dist = [];
    for (let i = 0; i < sz; i++) dist.push([100000, -1]);
